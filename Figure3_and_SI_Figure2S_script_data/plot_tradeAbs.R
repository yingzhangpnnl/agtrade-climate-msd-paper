# Ying Zhang plot circular graph of import and export under scenarios
require(tidyr)
library(dplyr)
library(stringr)
library(circlize)
...
# set plot color to be associated with regions
set.seed(99) #20
gridcol = rand_color(32+2)
colr <- Regmap["RegID1"] %>% 
  add_row(RegID1=c("Other regions","World")) %>% 
  cbind(gridcol)
...
scList <-unique(Agprod["scenario"]) # the listed scenarios correspond to RF, SC, SS, CC in the paper
savefignames <- c('Figure3S_RF.png','Figure3S_SC.png','Figure3S_SS.png','Figure3S_CC.png')
cropSel <- list("corn","othergrain","rice","wheat") # cereals

# for (i in seq(1,4)){
for (i in c(1,3)){
  rm(list=setdiff(ls(), c("i","scList","cropSel","Agdemand","Agprod","RegAgsource","colr","Regmap","savefignames")))
...
##############
#plot stars here
#for sum of crops selected above (cropSel) and the year defined below (yy)
...
nn = 10  #nn<31 regions
    ...
    colrSel<- data.frame(unique(dat_circular$REG_ex))
    colnames(colrSel)<-"RegID1"
    colrSel<-left_join(colrSel,colr)
    finalCol<-as.character(colrSel$gridcol)

    #save figure to default wd
      png(file = savefignames[i], width = 3500, height = 3500, res = 500)
